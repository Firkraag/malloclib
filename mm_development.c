...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    //     return NULL;
    // }
    // size_t old_ptr_size = GET_SIZE(HDRP(ptr));
    // PUT(HDRP(ptr), PACK(old_ptr_size, 0));
    // PUT(FTRP(ptr), PACK(old_ptr_size, 0));
    // char *block_ptr = coalesce(ptr);
    // size_t block_size = GET_SIZE(HDRP(block_ptr));
    // size_t required_block_size = QSIZE * ((size + QSIZE + (QSIZE - 1)) / QSIZE);
    // if (block_size >= required_block_size)
    // {
    //     // PUT(HDRP(block_ptr), PACK(block_size, 0));
    //     // PUT(FTRP(block_ptr), PACK(block_size, 0));
    //     place(block_ptr, required_block_size);
    //     memmove(block_ptr, ptr, old_ptr_size - QSIZE);
    //     return block_ptr;
    // }
    // if ((block_ptr = find_fit(required_block_size)) == NULL)
    // {
    //     if ((block_ptr = extend_heap(required_block_size)) == NULL)
    //         return NULL;
    // }
    // place(block_ptr, required_block_size);
    // memcpy(block_ptr, ptr, old_ptr_size - QSIZE);
    // // PUT(HDRP(ptr), PACK(old_ptr_size, 0));
    // // PUT(FTRP(ptr), PACK(old_ptr_size, 0));
    // // coalesce(ptr);
    // return block_ptr;
    size_t ptr_old_block_size = GET_SIZE(HDRP(ptr));
    if (ptr == NULL)
    {
        return mm_malloc(size);
    }
    ...
    size_t required_ptr_new_block_size = QSIZE * ((size + QSIZE + (QSIZE - 1)) / QSIZE);
    if (ptr_old_block_size - QSIZE >= size)
    {
        place(ptr, required_ptr_new_block_size);
        return ptr;
    }
    char *bp;
    /* Search the free list for a fit */
    ...
    if (GET_SIZE(HDRP(newptr)) >= size + QSIZE) {
        memcpy(newptr, ptr, ptr_old_block_size - QSIZE);
        return newptr;
    }
    if ((bp = find_fit(required_ptr_new_block_size)) == NULL)
    {
        if ((bp = extend_heap(required_ptr_new_block_size)) == NULL)
            return NULL;
    }
    place(bp, required_ptr_new_block_size);
    memcpy(bp, ptr, ptr_old_block_size - QSIZE);
    PUT(HDRP(ptr), PACK(ptr_old_block_size, 0));
    PUT(FTRP(ptr), PACK(ptr_old_block_size, 0));
    coalesce(ptr);
    return bp;
